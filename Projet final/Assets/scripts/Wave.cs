...
public class Wave : MonoBehaviour {
	...
	public void recupereMort(string mort, GameObject GOmort){
		...
		if (listMort.Count == nbMob) {
			foreach(GameObject tour in lesTourelles){
				tour.GetComponent<Unité>().setactivehades(false);
				tour.GetComponent<Unité>().setactivezeus(false);
				tour.GetComponent<Unité>().setactivegeb(false);
			}
	...
	void OnGUI(){
		...
		if(fin){
			GUI.Label (new Rect (450, 150, 100, 25), "Félicitation");
			if (GUI.Button (new Rect (300, 300, 100, 25), "Stage Clear")){
