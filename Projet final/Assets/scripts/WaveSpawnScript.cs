...
public class WaveSpawnScript : MonoBehaviour {
	...
	IEnumerator randSpawn (){
		...
		while (_nbMelee > 0 || _nbTank > 0 || _nbDistant > 0) {
			foreach (Transform meleeChild in _melee.transform) {
				if (_nbMelee > 0) {
					...
					_nbMelee--;
					LesMonstres.Add(melee.gameObject);
			...
			foreach (Transform tankChild in _tank.transform) {
				if (_nbTank > 0) {
					...
					_nbTank--;
					LesMonstres.Add(tank.gameObject);
					yield return new WaitForSeconds (2f);
				}
			}
			foreach (Transform distantChild in _distant.transform) {
				if (_nbDistant > 0) {
					...
					_nbDistant--;
					LesMonstres.Add(distance.gameObject);
					yield return new WaitForSeconds (2f);
				}
			}
		}
		if (_waveNumber % 5 == 0 ) 
		{
			nbWaveBoss = _waveNumber / 5;
		}
		while(nbWaveBoss > 0 ){
			Vector3 vecBoss = new Vector3(boss.transform.position.x, boss.transform.position.y, boss.transform.position.z);
			Transform bossCopie = Instantiate (boss.transform, vecBoss, Quaternion.identity) as Transform;
			bossCopie.gameObject.GetComponent<Adversaire>().setNom("_distant"+_nbDistant);
			bossCopie.gameObject.SetActive (true);
			_nbDistant--;
			LesMonstres.Add(bossCopie.gameObject);
			yield return new WaitForSeconds (2f);
			bossCopie.gameObject.SetActive(true);
			nbWaveBoss --;
		}
		...
		if (Network.peerType == NetworkPeerType.Disconnected) {
			...
			if (lancer == 0 ) {
				...
				if (Physics.Raycast (ray, out hit)) {
					if (Input.GetMouseButton (0) && (hit.collider.gameObject.name == "Unite(Clone)" || hit.collider.gameObject.name == "UniteArcher(Clone)" || hit.collider.gameObject.name == "UniteGolem(Clone)" || hit.collider.gameObject.name == "UniteGriffon(Clone)" || hit.collider.gameObject.name == "UniteMage(Clone)" || hit.collider.gameObject.name == "UniteSolo(Clone)" || hit.collider.gameObject.name == "UniteGuerrier(Clone)")) {
						amelio = true;
	...
	public ArrayList getLesMonstres(){
		return LesMonstres;
	}
	public void envoieBoss(){
		nbWaveBoss ++;
	}
}
